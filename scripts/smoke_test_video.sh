...
EXISTING_KEYFRAME_DIR="${EXISTING_KEYFRAME_DIR:-}"
...
args=(
  ...
  --storyboard-mode action_core
  --control-mode flf2v
  --keyframe-engine animagine
  --keyframe-frame-mode single_scene
  --keyframe-quality-preset anime_action_v2
  --shot-prompt-strength strong
  --composition-profile auto
  --motion-profile impact
  --postprocess frame_interpolate
  --target-fps 16
)

if [ -n "$EXISTING_KEYFRAME_DIR" ]; then
  args+=(--existing-keyframe-dir "$EXISTING_KEYFRAME_DIR")
fi

python3 "$PIPELINE_DIR/generate_video_sequence.py" "${args[@]}"
