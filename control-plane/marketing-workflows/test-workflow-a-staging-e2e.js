...
test('Workflow A staging E2E routes HerResearch and reaches Story Lock', () => {
  ...
  try {
    const seed = resolveEpisodeSeed('EP4'); const prepared = controller.prepare({ commandId: 'staging:a:prepare', workflow: 'research_and_story', runId, input: seed });
    ...
    const runRoot = path.join(root, runId); const body = { ...brief(), audience: seed.audience, pain_point: seed.pain_point, cta: seed.cta, evidence: ['episode-seed.json'] }; const briefBytes = Buffer.from(JSON.stringify(body, null, 2) + '\n'); const planBytes = Buffer.from(JSON.stringify(seed.capture_plan, null, 2) + '\n'); const assemblyBytes = Buffer.from(JSON.stringify(seed.assembly_plan, null, 2) + '\n'); fs.mkdirSync(runRoot, { recursive: true }); fs.writeFileSync(path.join(runRoot, 'production-brief.json'), briefBytes); fs.writeFileSync(path.join(runRoot, 'capture-plan.json'), planBytes); fs.writeFileSync(path.join(runRoot, 'assembly-plan.json'), assemblyBytes); const hash = crypto.createHash('sha256').update(briefBytes).digest('hex'); const planHash = crypto.createHash('sha256').update(planBytes).digest('hex'); const assemblyHash = crypto.createHash('sha256').update(assemblyBytes).digest('hex');
    const candidate = { schema_version: 'sdtk.video-task-result.v1', run_id: runId, task_id: 'research_story', attempt: 1, status: 'completed', artifacts: [{ path: 'production-brief.json', sha256: hash, media_type: 'application/json' }, { path: 'capture-plan.json', sha256: planHash, media_type: 'application/json' }, { path: 'assembly-plan.json', sha256: assemblyHash, media_type: 'application/json' }], validation: { status: 'pass', validator: 'workflow-a-staging-e2e-v1', evidence: ['production-brief.json', 'capture-plan.json', 'assembly-plan.json'] }, summary: 'Bounded research brief ready for owner review', error: null }; fs.writeFileSync(path.join(runRoot, 'worker-result.json'), JSON.stringify(candidate, null, 2) + '\n'); native.tasks.get(dispatch.native_task_id).status = 'done';
