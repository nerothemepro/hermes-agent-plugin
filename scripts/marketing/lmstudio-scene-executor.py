...
def schema(task, task_sha, model):
    media, motions, scene = task.get("allowed_media_ids", []), task.get("allowed_motion_ids", []), task["scene"]
    content = {"type":"object","additionalProperties":False,"required":["operation","motion_id","source_fragment","approved_media_ids"],"properties":{"operation":{"const":"propose_provider_fragment"},"motion_id":{"enum":motions},"source_fragment":{"type":"string","minLength":1,"maxLength":4000},"approved_media_ids":{"type":"array","maxItems":len(media),"uniqueItems":True,"items":{"enum":media}}}}
    fragment = {"type":"object","additionalProperties":False,"required":["scene_id","kind","content"],"properties":{"scene_id":{"const":scene["id"]},"kind":{"const":"provider_source_fragment"},"content":content}}
    return {"type":"object","additionalProperties":False,"required":["schema_version","task_sha256","model_id","fragments"],"properties":{"schema_version":{"const":RESULT_SCHEMA},"task_sha256":{"const":task_sha},"model_id":{"const":model},"fragments":{"type":"array","maxItems":1,"items":fragment},"notes":{"type":"string","maxLength":500}}}
...
def execute(file, model):
    ...
    prompt = "Return JSON only. Propose at most one HyperFrames source fragment for this locked task. Do not alter claims, CTA, facts, paths, URLs, credentials, shell commands, publish actions, or owner decisions. " + json.dumps(task, ensure_ascii=True, separators=(",", ":"))
    payload = {"model":model,"temperature":0,"messages":[{"role":"system","content":"You are a constrained local video scene assistant."},{"role":"user","content":prompt}],"response_format":{"type":"json_schema","json_schema":{"name":"sdtk_marketing_scene_result","strict":True,"schema":schema(task, task_sha, model)}}}
    try:
        ...
        with request.urlopen(req, timeout=timeout()) as response: content = json.loads(response.read().decode("utf-8"))["choices"][0]["message"]["content"]
